    ...
    <div id="Logs">
        ...
        <el-card class="box-card">
            ...
            <el-form :inline="true" size="mini" :model="queryMap" class="demo-form-inline">
                ...
                <el-form-item>
                    <el-button @click="deleteFileOrDirectory(sels)" :disabled="this.sels.length === 0"
    ...
    export default {
        ...
        methods: {
            async deleteFileOrDirectory() {
                ...
                if (res === "confirm") {
                    const {data: res} = await batchDelete("system/log/batchDelete/" + ids);
                    if (res.success) {
            ...
            //加载系统日志列表
            async getLogList() {
                const {data: res} = await findLogList(this.queryMap);
                if (!res.success) {
            ...
            async del(id) {
                ...
                if (res === "confirm") {
                    const {data: res} = await deleteLog("system/log/delete/" + id);
                    if (res.success) {
                        this.$message.success("系统日志删除成功");
                        await this.getLogList();
                    } else {
